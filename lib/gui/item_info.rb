...
module EH::GUI
  ...
  class ItemInfo < Element
    ...
    def initialize(x, y)
      super(x, y, 672, 160)
      @bg = EH.sprite("gui/iteminfo_background")
      @img = EH.sprite("items/none")
    ...
    def draw
      super
      @bg.draw(@x, @y, EH::GUI_Z + 9, @w/@bg.width.to_f, @h/@bg.height.to_f)
      @img.draw(@x, @y, EH::GUI_Z + 10, @h/@img.width.to_f, @h/@img.height.to_f)
      if @item
